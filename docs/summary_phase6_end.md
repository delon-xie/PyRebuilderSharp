...
Phase 6 全部 6 个子任务已完成。剩余未完成工作：

| 项目 | 优先级 | 类型 |
|:-----|:-------|:------|
| `match/case` ExceptionTable CFG 重建 | 🔴 高 | 语法覆盖 | opcode 映射完成，CFG 待重建 |
| `except*` ExceptionTable → IsGroup 映射 | 🔴 高 | 语法覆盖 | codegen 就绪，handler 集成待完成 |
| walrus 控制流检测（`if x := expr:` 模式） | 🟢 低 | 语法覆盖 | NamedExpr 基础检测完成 |
| AST 自动对比验证 | 🟡 中 | 工程增强 |
