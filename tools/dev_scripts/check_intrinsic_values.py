    ...
    if hasattr(c, 'co_name') and c.co_name == 'recursive_repr':
        for d in c.co_consts:
            if hasattr(d, 'co_name') and d.co_name == 'decorating_function':
                ...
                print(f"=== decorating_function bytecode ===")
                print(f"Code length: {len(code)}")
                
                # 查找 CALL_INTRINSIC_1 指令
                for i in range(0, len(code), 2):
                    op = code[i]
                    arg = code[i+1] if i+1 < len(code) else 0
                    
                    if op == 55:  # CALL_INTRINSIC_1 in Python 3.13
                        print(f'\nFound CALL_INTRINSIC_1 (op=55) at offset 0x{i:04X}, arg={arg}')
                        # 查看前后的指令
                        start = max(0, i-6)
