...
/// <summary>
/// Python 3.11 版本策略。
/// 3.11 是首个引入 cache/exceptiontable/localsplus/qualname 的版本。
/// 操作码编号与 3.12 部分不同（PRECALL_311=166, CALL_311=167, RESUME=90 等）。
/// cache 为稀疏模式（只有部分 opcode 有 cache）。
/// </summary>
public class VersionStrategy311 : VersionStrategyBase
{
    public override PythonVersion Version => PythonVersion.Py311;
    public override string DisplayName => "Python 3.11";

    // 3.11 使用 16 字节头部（PEP 552: magic + flags + hash + size）
    ...
    /// <summary>
    /// 将 Python 3.11 的原始 opcode 字节值映射到统一 Opcode 枚举。
    /// 3.11 特有的值：PRECALL_311=166, CALL_311=167, RESUME=90,
    /// POP_JUMP_IF_{TRUE,FALSE}=111/112。
    /// </summary>
    public override Opcode MapOpcode(byte rawOp)
    {
        return rawOp switch
        {
            // --- 所有版本一致的 (3.10 和 3.11 值相同) ---
            ...
            167 => Models.Bytecode.Opcode.CALL_311,
