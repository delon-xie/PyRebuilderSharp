...
result = subprocess.run(
    [os.path.expanduser("~/.pyenv/versions/2.7.18/bin/python"), "-c", 
     "import py_compile; py_compile.compile('/tmp/test_py27.py', cfile='/tmp/test_py27.pyc', doraise=True)"],
...
print("Full bytes:", ' '.join("{:02x}".format(b) for b in data))

# Also check what Python 2.7's imp.get_magic() returns
result2 = subprocess.run(
    [os.path.expanduser("~/.pyenv/versions/2.7.18/bin/python"), "-c", 
     "import imp; m = imp.get_magic(); print(' '.join('{:02x}'.format(ord(b)) for b in m))"],
    capture_output=True, text=True
)
