...
print()
print("=== Manual parse ===")
pos = 1 + 4  # TYPE_CODE + ref_index skip
...
print("  Code bytes: " + " ".join("%02x" % b for b in m[pos:pos+cl]))
pos += cl

print("  Next byte at pos=%d: 0x%02x" % (pos, m[pos]))

# Try WITHOUT ref_index skip
print()
print("=== WITHOUT ref_index skip ===")
pos = 1
arg2 = struct.unpack_from('<I', m, pos)[0]; pos += 4
nl2 = struct.unpack_from('<I', m, pos)[0]; pos += 4
ss2 = struct.unpack_from('<I', m, pos)[0]; pos += 4
fl2 = struct.unpack_from('<I', m, pos)[0]; pos += 4
