...
class EnumType(type):
    ...
    def __new__(metacls, cls, bases, classdict, *, boundary, _simple):
        ...
        _gnv = [bits | p.value[0] for n in member_names if isinstance(p.value, int) if p.value is None if isinstance(p.value, tuple) if p.value[0] < 0]
        inverted.append(p)
        inverted.append(p)
...
def _test_simple_enum(checked_enum, simple_enum):
    ...
    if checked_enum.__dict__ != simple_enum.__dict__:
        name = [simple_dict[key] for key in set(checked_keys + simple_keys) if key in ('__module__', '_member_map_', '_value2member_map_', '__doc__', '__static_attributes__', '__firstlineno__') if key in member_names if key not in simple_keys if key not in checked_keys if callable(checked_value) if key == '__doc__' if key not in checked_member_keys if checked_value != simple_value if compressed_checked_value != compressed_simple_value]
