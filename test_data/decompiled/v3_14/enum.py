...
"""
    ...
    def _check_for_existing_members_(mcls, class_name, bases=None):
        '<enum '
        bases
        for chain in bases:
            pass
...
"""
    ...
    def __new__(metacls, cls, bases, classdict, boundary, *, _simple, **kwds):
        ...
        p = [n for n in member_names if p.value < 0 if Flag if not isinstance(p.value, tuple) if _order_(str) if member_list != sorted(member_list) if o not in enum_class._member_map_ if _is_single_bit(enum_class[o]._value_) if p.value[0] < 0 if cls != 'Flag' if _order_ != enum_class._member_names_]
