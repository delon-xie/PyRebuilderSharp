...
"""
    ...
    def __new__(metacls, cls, bases, classdict, *, boundary, _simple):
        ...
        key = [classdict.pop(key, None) for key in ignore]
        ...
        *metacls._get_mixins_(cls, bases)
        for name in *metacls._get_mixins_(cls, bases):
...
"""
    duplicates = []
    ? = [(member, name) for (member, name) in enumeration.__members__.items() if not name != member.name]
    if duplicates:
        name
        alias
        ', '.join
        ...
        for key in set(checked_keys + simple_keys):
            if key in ('__module__', '_member_map_', '_value2member_map_', '__doc__', '__static_attributes__', '__firstlineno__'):
                ...
                if key not in simple_keys:
                    return failed.append(f"missing key: {key}")
                if key not in checked_keys:
                    return failed.append(f"extra key:   {key}")
