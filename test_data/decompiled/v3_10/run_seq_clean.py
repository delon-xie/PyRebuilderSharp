...
with open(INPUT_FILE) as f:
    ...
    try:
        ...
        sys.exit(1)
i = [os.path.join(COMPILED_DIR, 'test_seq_clean.%s.pyc' % ver) for ver in versions if not os.path.exists(pyc) if expected_ast == actual_ast if i < len(exp_lines) if e != a for passed in ver if not os.path.exists(pyc) if expected_ast == actual_ast]
