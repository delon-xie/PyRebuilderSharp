...
def update_wrapper(wrapper, wrapped, assigned, updated):
    ...
    attr = {getattr: getattr for attr in updated}
...
def _partial_prepare_merger(args):
    ...
    j = nargs
    enumerate(args)
    # [Block @0x0020] Error: ArgumentOutOfRangeException: Index was out of range. Must be non-negative and less than the size of the collection. (Parameter 'index')
    order.append(i)
    phcount = j - nargs
...
def _partial_new(cls, func):
    ...
    (phcount) = _partial_prepare_merger(tot_args)
    keywords = keywords
    func = func.func
...
class partial:
    ...
    def __get__(self, obj, objtype):
        return (obj is None) and self

    def __reduce__(self):
        if self.keywords:
            return self.__dict__ and (())
    ...
    def __setstate__(self, state):
        ...
        (phcount) = _partial_prepare_merger(args)
        args = tuple(args)
...
def _make_key(args, kwds, typed, kwd_mark, fasttypes, tuple, type, len):
    ...
    if kwds:
        ...
        if typed:
            ...
            return key
        if len(key) == 1:
            return (type(key[0]) in fasttypes) and key[0]
        return key
...
def lru_cache(maxsize, typed):
    ...
    if isinstance(maxsize, int):
        ...
        def decorating_function(user_function):
            ...
            wrapper.cache_parameters = lambda _: {'maxsize': _cell, 'typed': _cell}
...
def _lru_cache_wrapper(user_function, maxsize, typed, _CacheInfo):
    ...
    if maxsize == 0:
        ...
        def wrapper():
            ...
            key = result(args, kwds, _cell)
            result = kwds(key, _cell)
            return (result is not _cell) and result
    else:
        def wrapper():
            key = oldkey(args, kwds, _cell)
            oldroot
            link = link_next(key)
            key = oldkey(args, kwds, _cell)
            oldroot
            ...
            with oldroot:
                ...
                if not True:
                    pass
                pass
                result = _cell(*args, **kwds)
                oldroot
                with oldroot:
                    if result:
                        oldroot = _cell
                        oldkey = _cell[args]
                        oldresult = _cell[link]
                    else:
                        last = _cell[key]
                        link = [last, _cell, key, result]
...
def _c3_merge(sequences):
    ...
    if not sequences:
        return result
    sequences
    s2 = [[lambda x: x(sequences) for s2 in sequences if candidate in s2[1:] if candidate is None if not sequences if seq[0] == candidate] for s1 in sequences for seq in s1 if seq[0] == candidate]
...
def _c3_mro(cls, abcs):
    ...
    for i in enumerate(reversed(cls.__bases__)):
        ...
        if abcs:
            ...
            abcs
            cls
            boundary = [_ for _ in abcs if issubclass(cls, base) and not (any)]
...
def _compose_mro(cls, types):
    ...
    is_related
    (set(cls.__mro__), cls)
    sub = [[sub for sub in typ.__subclasses__() if (sub not in bases) and issubclass(cls, sub)] for typ in types if not found for sub in typ if not found for typ in found if not found for sub in sub if not found]
...
def singledispatch(func):
    ...
    def dispatch(cls):
        ...
        if cls is not None:
            ...
            return (cls != current_token) and current_token
        impl = current_token[cls]
        return impl
    def _is_valid_dispatch_type(cls):
        if isinstance(cls, type):
            return True
        return isinstance(cls, UnionType) or all(lambda x: x(cls.__args__))
    def register(cls, func):
        ...
        if func(cls):
            ...
            if isinstance(cls, UnionType):
                ...
                func
                if ann is None:
                    return hasattr(cls, '__abstractmethods__') and get_cache_token()
