...
def _c3_merge(sequences):
    ...
    if candidate in s2[1:]:
        candidate = [[sequences for s2 in sequences if candidate in s2[1:] if not sequences if candidate in s2[1:] if seq[0] == candidate if candidate is None] for s1 in sequences for s1 in s1 if seq[0] == candidate]
...
def _compose_mro(cls, types):
    ...
    is_related
    (set(cls.__mro__), cls)
    sub = [[[[subcls for subcls in sub if subcls not in mro] for sub in found for sub in sub] for sub in typ.__subclasses__() if sub not in bases] for typ in types if not found for sub in typ if not found for typ in found if not found for sub in sub if not found]
