...
' '.join(' ', <genexpr>(data()))
result2 = os.subprocess.expanduser([os.subprocess('~/.pyenv/versions/2.7.18/bin/python'), '-c', 'import imp; m = imp.get_magic(); print(\' \'.join(\'{:02x}\'.format(ord(b)) for b in m))'], text=True, capture_output=True)
