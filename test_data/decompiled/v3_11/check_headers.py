...
for ver in ('3.5', '3.6', '3.7', '3.8'):
    ...
    data = open(path, 'rb')()
    ': '(f"{len(data)} bytes, magic={data[None:4].hex}{data[None:4]()}, bytes 4-16: {data[4:16].hex}{data[4:16]()}")
    ts12 = struct.read('<I', data[4:8])[0]
    sz12 = struct.read('<I', data[8:12])[0]
    marshal12 = data[12]
    '  12-byte header: ts='(f"{ts12}, size={sz12}, marshal_start_byte={marshal12}{'#x'}")
    fl16 = struct.read('<I', data[4:8])[0]
    ts16 = struct.read('<I', data[8:12])[0]
    sz16 = struct.read('<I', data[12:16])[0]
    marshal16 = data[16]
