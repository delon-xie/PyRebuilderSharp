            ...
            for (name, obj) in cls.__dict__.items():
                if name[0] == '_':
                    ...
                    if isinstance(obj, property):
                        ...
                        return interesting.discard(name)
...
def unique(enumeration):
    ...
    """
    duplicates = []
    ? = [(name, member) for (name, member) in enumeration.__members__.items() if not name != member.name]
    if duplicates:
        name
        alias
        ', '.join
                    ...
                    if len(values) < 2:
                        ...
                        if enum_type == 'flag':
                            ...
                            if not missing:
                                ...
                                raise ValueError(f"invalid {enum_type!s} {cls_name!r}: missing values {', '.join(<genexpr>())!s}"[:256])
        ...
        for key in set(checked_keys + simple_keys):
            if key in ('__module__', '_member_map_', '_value2member_map_', '__doc__', '__static_attributes__', '__firstlineno__'):
                ...
                if key not in simple_keys:
                    return failed.append(f"missing key: {key!r}")
                if key not in checked_keys:
                    return failed.append(f"extra key:   {key!r}")
