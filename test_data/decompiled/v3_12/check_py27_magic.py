...
import os
import subprocess
result = subprocess.run([os.path.expanduser('~/.pyenv/versions/2.7.18/bin/python'), '-c', 'import py_compile; py_compile.compile(\'/tmp/test_py27.py\', cfile=\'/tmp/test_py27.pyc\', doraise=True)'], True, True)
print('Compile result:', result.stdout, result.stderr)
data = open('/tmp/test_py27.pyc', 'rb').read()
print('Length:', len(data))
'Full bytes:'(' '.join, <genexpr>(data()))
result2 = subprocess.run([os.path.expanduser('~/.pyenv/versions/2.7.18/bin/python'), '-c', 'import imp; m = imp.get_magic(); print(\' \'.join(\'{:02x}\'.format(ord(b)) for b in m))'], True, True)
