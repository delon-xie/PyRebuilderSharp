...
def main():
    results = []
    pyc_files = sorted(TEST_DATA_DIR.glob("*.pyc"))
    total_files = len(pyc_files)
    
    print(f"Found {total_files} .pyc files")
    
    for i, pyc_path in enumerate(pyc_files, 1):
        print(f"[{i}/{total_files}] Processing {pyc_path.name}")
        
        version = extract_version(pyc_path.name)
